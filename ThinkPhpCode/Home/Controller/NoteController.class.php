...
class NoteController extends CommonController
{
    ...
    public function index()
    {
        ...
        if (empty($noteCount)) {
            ...
            $note = D('Note');
            //查询数据库所有文章数据
            $res = $note->field('type,date')->select();
    ...
    public function lists()
    {
        ...
        if (!in_array($type, $this->list_type)) {
            ...
            if ($note->count() >= 1) {
                //根据文章类型降序查询
                $res = $note->order('id desc')->where("type='$type'")->select();
                //获取元素总数
                $count = count($res);
                //判断元素总数
                if ($count >= 1) {
                    ...
                    for ($i = 0; $i < 8; $i++) {
                        ...
                        $classNum[$this->list_type[$rand]] = count($note->where(['type' => $this->list_type[$rand]])->select());
                    }
                    //实例化文章评论表模型
                    $dis = D('NoteDiscuss');
                    //开始查询子表信息
                    foreach ($res as $k => $v) {
                        //查询子表中对应的评论总数,装入数组
                        $res[$k]['discussNum'] = count($dis->where(['pids' => $v['id'], 'status' => 1])->select());
                    }
                    //传入模板
    ...
    public function article()
    {
        ...
        //查询该文章的数据
        $res = $note->where(['name' => $name, 'type' => $type])->select()[0];
        //增加此文章的查看次数
        $res['view'] += 1;
        ...
        //获取IP并加密
        $userIp=substr(md5($name.get_client_ip()),0,10);

        //获取该文章的缓存数据
        $CacheData=S($userIp);
        if(empty($CacheData)){
            //将Note表数据写入缓存
            $CacheData['noteRes']=$res;

            //实例化文章评论表模型
            $dis=D('NoteDiscuss');
            //查询该文章评论数据
            $discuss = $dis->field('name,content,date')->order('id desc')->where(['pids' => $res['id'], 'status' => 1])->select();
            //获取评论总数装入数组
            $discuss['count'] = count($discuss);


            //查询并计算该分类的总数
            $classNum = $note->field('count(*) as typenum')->where("type='" . $res['type'] . "'")->select()[0]['typenum'];
            //定义每页的条数
            $n = 6;
            //进一法计算该分类的总页数
            ...
            //取出指定页的推荐数据
            $recommend = $note->field('name')->page($rand, $n)->where("type='" . $res['type'] . "'")->select();

            //如果该用户没有查看过
            if(!$_SESSION['note_view_'.$name]) {
                //增加查看数量
                $view = ((int)$res['view']);
                $note->execute("update h_note set view=$view where id=" . $res['id']);
                $_SESSION['note_view_'.$name]='ok';
            }
            $CacheData['discuss']=$discuss;
            $CacheData['recommend']=$recommend;
            S($userIp,$CacheData,300);
        }

        //数据传入模板
        $this->assign('res', $CacheData['noteRes']);
        $this->assign('discuss', $CacheData['discuss']);
        $this->assign('recommend', $CacheData['recommend']);
        $this->display();
    }
    ...
    public function discuss()
    {
        ...
        $note = D('Note');
        //根据该文章名称获取id
        $parentID = $note->field('id')->where("name='$textName' and type='$type'")->select()[0]['id'];
        //获取当前用户ip
        $ip = get_client_ip();
        //实例化文章评论表模型
        $dis=D('NoteDiscuss');
        //查询当前用户是否已评论过该文章
        if ($dis->where(['pids' => $parentID, 'uip' => $ip])->select()) {
            //返回错误代码
            echo -3;
            //终止程序
            die;
